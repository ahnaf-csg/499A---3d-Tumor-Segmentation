...
# --------------------------------------------------------------------------- #
# Boundary-halo analysis
# --------------------------------------------------------------------------- #

def halo_analysis(per_case: list[dict], threshold: float = 0.5,
                  region: str = "ET", thresholds=None) -> dict:
    """Test whether over-segmentation is a constant-thickness boundary shell.

    HYPOTHESIS. The model surrounds each lesion with a low-confidence halo of
    roughly constant absolute thickness t. For a lesion of radius r,

        V_pred / V_gt  ~  (1 + t/r)^3 ,    r  ~  (3 V_gt / 4pi)^(1/3)

    so the volume ratio should FALL monotonically with lesion size: negligible
    around a large lesion, dominant around a small one. That single mechanism
    would explain both the over-segmentation (recall >> precision) and the
    size-dependent Dice collapse.

    Fits t by least squares on r_pred - r_gt, both derived from volumes under a
    sphere-equivalent approximation, and reports the correlation between ratio
    and size as the actual test of the hypothesis.

    Expects records carrying 'gt_volume_cm3' and either 'ET_pred_vol' (an array
    over thresholds, from calibrate.sweep_streaming) or a scalar
    '<region>_pred_volume_cm3'.
    """
    vg, vp = [], []
    for c in per_case:
        if c.get("gt_empty", {}).get(region, False) or not c.get("gt_volume_cm3"):
            continue
        key = f"{region}_pred_vol"
        if key in c and thresholds is not None:
            ti = list(thresholds).index(threshold)
            p = float(np.asarray(c[key])[ti])
        elif f"{region}_pred_volume_cm3" in c:
            p = float(c[f"{region}_pred_volume_cm3"])
        else:
            continue
        if p <= 0:
            continue
        vg.append(float(c["gt_volume_cm3"]))
        vp.append(p)

    if len(vg) < 8:
        return {"n": len(vg), "error": "too few cases with both volumes"}

    vg, vp = np.array(vg), np.array(vp)
    ratio = vp / vg
    # sphere-equivalent radii in mm (volumes are cm^3 -> x1000 mm^3)
    r_gt = (3 * vg * 1000 / (4 * np.pi)) ** (1 / 3)
    r_pred = (3 * vp * 1000 / (4 * np.pi)) ** (1 / 3)
    t_per_case = r_pred - r_gt

    # the hypothesis test: does the ratio fall as lesions get bigger?
    lr, lv = np.log(ratio), np.log(vg)
    rho = float(np.corrcoef(lv, lr)[0, 1])
    slope, intercept = np.polyfit(lv, lr, 1)

    out = {
        "n": len(vg), "threshold": threshold, "region": region,
        "mean_volume_ratio": round(float(ratio.mean()), 4),
        "median_volume_ratio": round(float(np.median(ratio)), 4),
        "halo_thickness_mm_mean": round(float(t_per_case.mean()), 3),
        "halo_thickness_mm_median": round(float(np.median(t_per_case)), 3),
        "halo_thickness_mm_sd": round(float(t_per_case.std()), 3),
        "log_ratio_vs_log_volume_r": round(rho, 4),
        "log_ratio_vs_log_volume_slope": round(float(slope), 4),
        "verdict": ("consistent with a constant-thickness halo: the volume ratio "
                    "falls significantly as lesions grow" if rho < -0.3 else
                    "NOT consistent with a constant-thickness halo: ratio is "
                    "roughly independent of size"),
    }

    # per-bin, so the pattern is legible in a table
    out["by_volume"] = []
    for lo, hi in ((0, 2), (2, 5), (5, 15), (15, 1e9)):
        m = (vg >= lo) & (vg < hi)
        if m.sum():
            out["by_volume"].append({
                "bin_cm3": f"{lo}-{'inf' if hi > 1e8 else hi}", "n": int(m.sum()),
                "mean_gt_cm3": round(float(vg[m].mean()), 3),
                "mean_pred_cm3": round(float(vp[m].mean()), 3),
                "volume_ratio": round(float(ratio[m].mean()), 3),
                "halo_mm": round(float(t_per_case[m].mean()), 3)})
    return out
